class Glyph
  ...
  def self.populate!
    populate_from_wowhead "http://wod.wowhead.com/items=16.4"
  end
  ...
  def self.populate_from_wowhead(url, options = {})
    ...
    ids.each do |id|
      doc = open("http://wod.wowhead.com/item=%d&xml" % id, 'User-Agent' => 'Mozilla/5.0 (Windows NT 6.3; WOW64; rv:27.0) Gecko/20100101 Firefox/27.0').read
      xml = Nokogiri::XML(doc)
      json = JSON::load("{%s}" % xml.css("json").text)
      puts json.inspect

      if matches = doc.match(/spell=(\d+)\"/)
        spellDoc = open("http://wod.wowhead.com/spell=%d" % matches[1], 'User-Agent' => 'Mozilla/5.0 (Windows NT 6.3; WOW64; rv:27.0) Gecko/20100101 Firefox/27.0').read
