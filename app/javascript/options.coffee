class ShadowcraftOptions
  ...
  initOptions: ->
    ...
    @setup("#settings section.mutilate .settings", "rotation", {
      min_envenom_size_mutilate: {type: "select", name: "Min CP/Envenom > 35%", options: [5,4,3,2,1], 'default': 4, desc: "Use Envenom at this many combo points, when your primary CP builder is Mutilate", datatype: 'integer'}
      min_envenom_size_backstab: {type: "select", name: "Min CP/Envenom < 35%", options: [5,4,3,2,1], 'default': 5, desc: "Use Envenom at this many combo points, when your primary CP builder is Backstab", datatype: 'integer'}
    ...
    @setup("#settings section.combat .settings", "rotation", {
      ...
      ksp_immediately: {type: "select", name: "Killing Spree", options: {'true': "Killing Spree on cooldown", 'false': "Wait for Bandit's Guile before using Killing Spree"}, 'default': 'false', datatype: 'bool'}
